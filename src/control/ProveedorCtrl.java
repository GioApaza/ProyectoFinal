...
public class ProveedorCtrl implements Control<Proveedor>{  // implementa los metodos de control para cliente a interfase insert search delete update
		...
		public ArrayList<Proveedor> list() throws Throwable { //arraylist es una estructura de tipo arreglo con tamaño indefinido es una estructura dinamica
			...
			while (rs.next()) { // while para que vaya recoriendo y nos devuelva todos los elementos del rs 
				...
				proveedores.add(new Proveedor(codigoProveedor, nombre, telefono, direccion)); // añade cliente nuevo 
			}
...
System.out.println("Entro metodo");
			conexion.SQL("Insert into proveedor(codigoProveedor,nombre, telefono,direccion) VALUES(?,?,?,?)");//?? son parametros esta esperando dos parametros
			...
			conexion.preparedStatement().setString(4, proveedor.getDireccion());
			conexion.preparedStatement().setInt(3, proveedor.getTelefono());
			conexion.CUD();
		...
		public Proveedor search1(Proveedor proveedor) throws Throwable { // recibe el cliente y va buscar la clave del cliente y devulve el nombre a ese cliente y lo cierra
			Proveedor prov= null;
			ResultSet rs;

			conexion.SQL("Select * from proveedor where codigoProveedor=?");
			conexion.preparedStatement().setInt(1, proveedor.getCodigoProveedor());
			rs = conexion.resultSet();

			while (rs.next()) {
				System.out.println("Encontro");
				prov = new Proveedor(proveedor.getCodigoProveedor(),rs.getString("nombre"),rs.getInt("telefono"),rs.getString("direccion"));
			}
...
return prov;
		...
		public void update1(Proveedor proveedor) throws Throwable { // modifica datos del cliente con el nit
			...
			if (proveedor != null) {
				...
				conexion.SQL("Update proveedor set nombre = ?,direccion = ?,telefono= ? where codigoProveedor=?");
		...
		@Override
		public void search(Proveedor entity) throws Throwable {
			// TODO Auto-generated method stub
